...
if (array_key_exists('stat', $_GET)) {
    ...
    if ($user->getStat('upgrade_points') <= 0) {
        ...
        $sql = "SELECT `stat_name` FROM `stats` WHERE `in_gym`=1 AND `stat_id`=$stat_id";
...
<table class="table table-bordered">
    ...
    <?php
    //Get stats
    $sql = "SELECT `stat_id`,`stat_name` FROM `stats` WHERE `in_gym`=1";
    $get_stats = $db->prepare($sql);
    $get_stats->execute();
    $get_stats->bind_result($stat_id, $stat_name);
    $get_stats->store_result();
    while ($get_stats->fetch()) {
        echo '<tr>  <td>' . ucwords($stat_name) . '</td> 
                    <td>' . number_format($user->getStat($stat_name)) . '</td>
